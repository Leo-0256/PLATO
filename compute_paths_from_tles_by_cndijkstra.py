...
def compute_paths_from_tles_by_cndijkstra(tles_filepath, time_str, out_filepath):
    ...
    lib = ctypes.cdll.LoadLibrary("./cndijkstra.so")
    lib.cndijkstra.restype = ctypes.c_void_p
    ...
    lib.cndijkstra(num_orbits, num_sat_per_orbit, cpp_one_dimensional_array)
    ...
    print(
        "cndijkstra Calculation Distance Matrix Time By CPP: ",
        (endtime - starttime).total_seconds(),
        "s",
    )

    # with open("./2b.txt", "w") as fff:
    #     for i in range(num_satellites):
    #         for j in range(num_satellites):
...
if __name__ == "__main__":
    ...
    time_str = "2000-01-01 00:01:00"
    compute_paths_from_tles_by_cndijkstra(tles_filepath, time_str, "PATH/starlink_tle.txt")
