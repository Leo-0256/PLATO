...
def compute_paths_from_tles(tles_filepath, time_str, out_filepath):
    ...
    for i in range(num_satellites):
        ...
        distance = np.minimum(
            distance, distance[i, :][np.newaxis, :] + distance[:, i][:, np.newaxis]
        )
...
if __name__ == "__main__":
    ...
    # Initialize Topology Time:  0.11513 s
    # Floyd Calculation Distance Matrix Time:  8.707908 s
    # Predecessor Matrix Update Time:  0.840879 s
